...
static const CGFloat kLGTabBarPortraitLensHeight = 64.0;
...
static void LGSetTabBarLensMask(LGLiveBackdropView *lens, CGRect shape,
                                CGFloat radius) {
    if (!lens || CGRectIsEmpty(shape)) return;
    CAShapeLayer *mask = objc_getAssociatedObject(lens, kLGTabBarLensMaskKey);
    if (!mask) {
        mask = [CAShapeLayer layer];
        mask.fillColor = UIColor.whiteColor.CGColor;
        objc_setAssociatedObject(lens, kLGTabBarLensMaskKey, mask,
                                 OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    }
    mask.frame = lens.bounds;
    CGPathRef path = CGPathCreateWithRoundedRect(shape, radius, radius, NULL);
    mask.path = path;
    CGPathRelease(path);
    lens.layer.mask = mask;
}
...
static void LGSetTabBarLensPill(UITabBar *bar, LGLiveBackdropView *lens,
    ...
    if (LGTabBarUsesLandscapeMetrics(bar)) {
        ...
        lens.lgShapeRect = lens.bounds;
        lens.lgShapeCornerRadius = CGRectGetHeight(pill) * 0.5;
        LGSetTabBarLensMask(lens, lens.bounds, CGRectGetHeight(pill) * 0.5);
        [CATransaction commit];
        LGLensRectWrite(LGLensRectSlotTabBarSelection, true, 0.0, 0.0, 1.0, 1.0);
        return;
    }
    ...
    LGSetTabBarLensMask(lens, pillInLens,
                        CGRectGetHeight(pillInLens) * 0.5);
    [CATransaction commit];
...
static void LGSyncTabBarRestingSelection(UITabBar *bar);
...
static UIColor *LGTabBarResolveAccent(UITabBar *bar) {
    ...
    if (item) {
        NSDictionary *attrs = [item titleTextAttributesForState:UIControlStateSelected];
        UIColor *itemColor = attrs[NSForegroundColorAttributeName];
        if ([itemColor isKindOfClass:UIColor.class]) return itemColor;
    }
    if (@available(iOS 13.0, *)) {
        UITabBarItemStateAppearance *selected =
            bar.standardAppearance.stackedLayoutAppearance.selected;
        if (selected.iconColor) return selected.iconColor;
        UIColor *title = selected.titleTextAttributes[NSForegroundColorAttributeName];
        if ([title isKindOfClass:UIColor.class]) return title;
    }
...
static void LGApplyTabBarGlyphColor(UITabBar *bar, UIColor *color) {
    ...
    if (![accent isKindOfClass:UIColor.class]) accent = LGTabBarResolveAccent(bar);
...
static void LGSampleTabBarLuma(UITabBar *bar) {
    ...
    if (!pixel) { if (data) CFRelease(data); return; }
...
static void LGStartTabBarLumaSampling(UITabBar *bar) {
    ...
    NSTimer *timer = [NSTimer timerWithTimeInterval:1.0 repeats:YES block:^(__unused NSTimer *unused) {
        UITabBar *strongBar = weakBar;
        if (!strongBar || !strongBar.window || strongBar.hidden || strongBar.alpha < 0.01 ||
            [objc_getAssociatedObject(strongBar, kLGTabBarInteractionActiveKey) boolValue] ||
...
static UIImageView *LGTabBarInnerGlow(UITabBar *bar, BOOL create) {
    ...
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        s_glowImage = LGTabBarCreateGlowImage(144.0);
    });

    glow = [[UIImageView alloc] initWithImage:s_glowImage];
    glow.bounds = CGRectMake(0.0, 0.0, 144.0, 144.0);
...
static void LGConfigureTabBarAppearance(UITabBar *bar) {
    ...
    if (@available(iOS 13.0, *)) {
        ...
        else if ([selected.titleTextAttributes[NSForegroundColorAttributeName]
                  isKindOfClass:UIColor.class])
            accent = selected.titleTextAttributes[NSForegroundColorAttributeName];
    }
    if (!accent) accent = UIColor.labelColor;
    ...
    if (@available(iOS 13.0, *)) {
        ...
        for (NSUInteger i = 0; i < 3; i++) {
            ...
            state.normal.iconColor = neutral;
            NSMutableDictionary *normalAttrs = [state.normal.titleTextAttributes mutableCopy]
                ?: [NSMutableDictionary dictionary];
            normalAttrs[NSForegroundColorAttributeName] = neutral;
            state.normal.titleTextAttributes = normalAttrs;
            state.selected.iconColor = accent;
            NSMutableDictionary *selectedAttrs = [state.selected.titleTextAttributes mutableCopy]
                ?: [NSMutableDictionary dictionary];
            selectedAttrs[NSForegroundColorAttributeName] = accent;
...
static LGLiveBackdropView *LGTabBarSelectionLens(UITabBar *bar) {
    ...
    lens.hidden = YES;
    lens.alpha = 0.0;
    /* Use the host preference; the renderer provides the real bezel/refraction edge. */
    lens.lgSpecularEnabledOverride = nil;
    lens.layer.masksToBounds = YES;
    lens.clipsToBounds = YES;
...
static void LGSyncTabBarRestingSelection(UITabBar *bar) {
    ...
    if (state && (state.active || state.awaitingTapDestination ||
                  state.awaitingRestingShape)) return;
    ...
    if (selectedIndex == NSNotFound || selectedIndex >= buttons.count) {
        ...
        LGLensRectWrite(LGLensRectSlotTabBarSelection,
                        false, 0.0, 0.0, 0.0, 0.0);
        return;
    }

    if (!lens) lens = LGTabBarSelectionLens(bar);
    if (!lens) return;

    UIView *selectedButton = buttons[selectedIndex];
    CGRect buttonFrame =
        [selectedButton.superview convertRect:selectedButton.frame toView:bar];
    CGFloat height = LGTabBarHighlightHeight(bar);
    CGRect resting = CGRectMake(
        CGRectGetMidX(buttonFrame) - CGRectGetWidth(buttonFrame) * 0.5,
        CGRectGetMidY(LGTabBarPillFrame(bar)) - height * 0.5,
        CGRectGetWidth(buttonFrame), height);
    LGSetTabBarLensPill(bar, lens, resting);
    lens.transform = CGAffineTransformIdentity;
    lens.alpha = 1.0;
    lens.hidden = NO;
    NSUInteger firstButtonIndex = NSNotFound;
    for (UIView *button in buttons) {
        NSUInteger index = [bar.subviews indexOfObjectIdenticalTo:button];
        if (index != NSNotFound) firstButtonIndex =
            MIN(firstButtonIndex, index);
    }
    if (firstButtonIndex != NSNotFound)
        [bar insertSubview:lens atIndex:firstButtonIndex];
}
...
static void LGBeginTabBarLiquidMotion(UITabBar *bar,
    ...
    CGRect existingPill = LGTabBarLensPillRect(lens);
    BOOL hasExistingPill = !CGRectIsEmpty(existingPill) &&
                           CGRectGetWidth(existingPill) > 0.0 &&
                           CGRectGetHeight(existingPill) > 0.0;
    CGFloat centerX = CGRectGetMidX(frame);
    CGFloat restingCenterX = hasExistingPill
        ? CGRectGetMidX(existingPill) : CGRectGetMidX(buttonFrame);
    CGFloat restingWidth = hasExistingPill
        ? CGRectGetWidth(existingPill) : CGRectGetWidth(buttonFrame);
    CGFloat restingHeight = hasExistingPill
        ? CGRectGetHeight(existingPill) : LGTabBarHighlightHeight(bar);
    state.bar = bar;
    state.lens = lens;
    state.generation += 1;
    state.awaitingTapDestination = NO;
    ...
    state.renderedHeight = restingHeight;
...
static void LGShowTabBarSelectionLens(UITabBarButton *button, UITouch *touch) {
    ...
    objc_setAssociatedObject(bar, kLGTabBarInteractionActiveKey, @YES,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    UIColor *accent = LGTabBarResolveAccent(bar);
    objc_setAssociatedObject(bar, kLGTabBarAccentColorKey, accent,
                             OBJC_ASSOCIATION_RETAIN_NONATOMIC);
    UIColor *neutralTint = bar.unselectedItemTintColor ?: LGTabBarNeutralGlyphColor(bar);
    NSUInteger visualIndex = [LGStockTabBarButtons(bar) indexOfObjectIdenticalTo:button];
    if (visualIndex != NSNotFound) LGSetTabBarVisualAccentIndex(bar, visualIndex);
...
static void LGMoveTabBarSelectionLens(UITabBarButton *button, UITouch *touch) {
    ...
    if (state.dragged) {
        ...
        NSNumber *current = objc_getAssociatedObject(bar, kLGTabBarVisualAccentIndexKey);
...
static void LGFinalizeTabBarSelection(UITabBar *bar,
    ...
    objc_setAssociatedObject(bar, kLGTabBarAccentColorKey, nil,
                             OBJC_ASSOCIATION_ASSIGN);
    objc_setAssociatedObject(bar, kLGTabBarInteractionActiveKey, nil,
                             OBJC_ASSOCIATION_ASSIGN);
    [state stop];
    LGSyncTabBarRestingSelection(bar);
    LGStyleStockTabBar(bar);
